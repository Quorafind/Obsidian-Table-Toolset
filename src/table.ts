...
export const handleRenderMethod = (self: TableToolsPlugin, data: TableCell, updateTrigger: () => void) => {
	...
	if (dataText && dataTextContainsBr) {
		...
		if (data.table) self.patchedTable.add(data.table);

		MarkdownRenderer.render(data.table.app, text, data.contentEl, data.table.editor.view.file.path, data.table.editor).then(() => {
