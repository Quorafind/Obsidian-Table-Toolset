...
export async function updateTableFromData(dataArray: string[][], tableElement: HTMLTableElement, app: App, plugin: TableToolsPlugin, ctx: MarkdownPostProcessorContext): Promise<void> {
	tableElement.toggleClass('table-rendered', true);
	tableElement.parentElement?.toggleClass('table-wrapper', true);

	for (let i = 0; i < dataArray.length; i++) {
		const row = dataArray[i];
		for (let j = 0; j < row.length; j++) {
			const cellData = row[j];
			const tableCell = tableElement.rows[i].cells[j];

			if (cellData.replaceAll('<br>', '\n').includes('\n')) {
				tableCell.empty();
				const contentEl = tableCell.createEl('div', {
					cls: 'table-cell-wrapper markdown-rendered',
				});
				await MarkdownRenderer.render(
					app, cellData.replaceAll('<br>', '\n'), contentEl, ctx.sourcePath, plugin
				);
			} else {
				tableCell.empty();
				tableCell.createEl('div', {
					cls: 'table-cell-wrapper',
					text: cellData,
				});
			}
		}
...
export function markdownTableToArray(markdown: string): string[][] {
	...
	return array.filter((line, index) => {
		return !(index === 1 && line.every(cell => cell.match(/^[-: ]+$/)));
	});
}
